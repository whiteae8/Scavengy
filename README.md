...
- **Landmark verification & geocoding** — every AI-suggested landmark is looked up against the Google Places API; clues are dropped if the landmark can't be found, resolves to the wrong state/region (a "Glenview, KY" hunt won't accept a match in Glenview, IL), or falls outside a 15-mile radius of the hunt's own geocoded center (haversine distance check). Small towns without enough standalone landmarks are allowed to borrow from nearby towns in the *same* region — the region check and the distance check are deliberately separate, so that flexibility doesn't reopen the door to a same-named place three states away
...
- **AI output is verified against ground truth before it's persisted.** The model is treated as a proposer, not an authority: every landmark it suggests must geocode successfully via Google Places, match the expected state/region (address components are checked to catch same-name-place collisions across regions — city-level differences are intentionally allowed here), and sit within a 15-mile haversine radius of the hunt's own geocoded center. Clues that fail any check are dropped and logged, and the surviving clues are re-indexed — hallucinated or misplaced landmarks never reach the database.
...
- **Client-side key exposure is a scoped, deliberate tradeoff, not an oversight.** The Place Picker and Maps JS API are Google web components that run in the browser and require a visible API key — for a project with no public deployment, that exposure is an accepted risk rather than a real one. The server-side Places key (used for landmark geocoding in `HuntService`) is entirely separate and never leaves the backend; if this were deployed publicly, the browser-facing key would just need an HTTP-referrer restriction in Cloud Console, no code changes required.
